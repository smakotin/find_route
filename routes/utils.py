...
def dfs_paths(graph, start, goal):
    """Функция поиска всех возможных
    маршрутов из одного города в другой. Вариант
    посещения из одного и того же города более
...
def get_routes(request, form) -> dict:
    ...
    for route in right_ways:
        ...
        if total_time <= travelling_time:
            routes.append(tmp)
    if not routes:
        raise ValueError(f'Есть маршрут, но c большим временем в пути'
                         f'Укажите время {total_time}, чтобы отразить все возможные маршруты')
    sorted_routes = []
    if len(routes) == 1:
